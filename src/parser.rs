...
fn parse_plain_id(id: &str, default_date: Option<DateTime<Utc>>) -> Result<ParsedPatient, NetworkError> {
    if id.trim().is_empty() {
...
fn parse_aeh_id(id: &str) -> Result<ParsedPatient, NetworkError> {
    ...
    if parts.is_empty() || parts[0].trim().is_empty() {
        return Err(NetworkError::Format(format!("Invalid AEH format for ID: {}", id)));
    }
...
fn parse_lanl_id(id: &str) -> Result<ParsedPatient, NetworkError> {
    ...
    if parts.len() < 3 {
        return Err(NetworkError::Format(format!("Invalid LANL format for ID: {}. Expected at least 3 parts separated by '_'", id)));
    }
...
fn parse_regex_id(id: &str, default_date: Option<DateTime<Utc>>) -> Result<ParsedPatient, NetworkError> {
...
pub fn parse_date(date_str: &str) -> Result<DateTime<Utc>, NetworkError> {
    ...
    for format in formats.iter().filter(|f| !f.contains("%H:%M:%S")) {
        if let Ok(dt) = NaiveDateTime::parse_from_str(&format!("{} 00:00:00", date_str), &format!("{} %H:%M:%S", format)) {
            return Ok(DateTime::<Utc>::from_naive_utc_and_offset(dt, Utc));
        }
    }
    ...
    Err(NetworkError::Format(format!("Unable to parse date: {}", date_str)))
}