...
fn test_basic_network_construction() {
    ...
    assert!(
        network.get_edge_count() > 0,
        "Should have edges with distance <= 0.03"
    );
    ...
    for id in &["ID1", "ID2", "ID3"] {
        assert!(
            id1_cluster_nodes.contains(&id.to_string()),
            "{} should be in the same cluster as ID1",
            id
        );
    }
    ...
    assert_ne!(
        id1_cluster_id, id6_cluster_id,
        "ID1 and ID6 should be in different clusters"
    );
    ...
    for id in &["ID6", "ID7", "ID8"] {
        assert!(
            id6_cluster_nodes.contains(&id.to_string()),
            "{} should be in the same cluster as ID6",
            id
        );
    }
}
...
fn test_id_formats() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse AEH format CSV: {:?}",
        result.err()
    );
    ...
    assert!(
        ...
        "Should have parsed some nodes from AEH format"
    );
    ...
    assert!(
        result.is_ok(),
        "Failed to parse LANL format CSV: {:?}",
        result.err()
    );
    ...
    assert!(
        network.get_node_count() > 0,
        "Should have parsed some nodes from LANL format"
    );
}
...
fn test_duplicate_edges() {
    ...
    assert!(
        result.is_ok(),
        "Failed to parse CSV with duplicate edges: {:?}",
        result.err()
    );
    ...
    assert_eq!(
        ...
        2,
        "Should deduplicate edges and keep 2 unique edges"
    );

    // Check that we have deduplicated the edges
    assert!(
        network.get_edge_count() <= 2,
        "Should have maximum 2 unique edges after deduplication"
    );
}
...
fn test_json_output_format() {
    ...
    assert!(
        json_output.contains("\"trace_results\""),
        "JSON should have trace_results field"
    );
    assert!(
        json_output.contains("\"Network Summary\""),
        "JSON should have Network Summary"
    );
    assert!(
        json_output.contains("\"Nodes\""),
        "JSON should have Nodes field"
    );
    assert!(
        json_output.contains("\"Edges\""),
        "JSON should have Edges field"
    );
    assert!(
        json_output.contains("\"Cluster sizes\""),
        "JSON should have Cluster sizes field"
    );
    ...
    let node_ids = parsed_json["trace_results"]["Nodes"]["id"]
        .as_array()
        .unwrap();
    assert!(!node_ids.is_empty(), "JSON should include nodes");

    // Check that edges are in the JSON
    let edge_sources = parsed_json["trace_results"]["Edges"]["source"]
        .as_array()
        .unwrap();
    assert!(!edge_sources.is_empty(), "JSON should have edges");

    // Verify thresholds
    let threshold = parsed_json["trace_results"]["Settings"]["threshold"]
        .as_f64()
        .unwrap();
...
fn test_network_stats() {
    ...
    assert!(
        stats.get("nodes").and_then(|v| v.as_u64()).unwrap() > 0,
        "Stats should have nodes"
    );
    assert!(
        stats.get("edges").and_then(|v| v.as_u64()).unwrap() > 0,
        "Stats should have edges"
    );
    assert_eq!(
        stats.get("clusters").and_then(|v| v.as_u64()),
        Some(2),
        "Stats should have 2 clusters"
    );
