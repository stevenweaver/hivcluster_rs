...
fn test_singleton_detection() {
    ...
    assert_eq!(
        singletons.len(),
        4,
        "Should identify 4 singletons (ID7, ID8, ID9, ID10)"
    );
    for id in &["ID7", "ID8", "ID9", "ID10"] {
        assert!(
            singletons.contains(&id.to_string()),
            "{} should be identified as a singleton",
            id
        );
    }
    ...
    assert_eq!(
        summary.Singletons, 4,
        "JSON should report 4 singleton nodes"
    );
    ...
    assert_eq!(
        summary.sequences_used, 6,
        "JSON should report 6 nodes used in connections"
    );
    ...
    assert!(
        degree_distribution.len() > 0,
        "Should have a degree distribution"
    );
    assert_eq!(
        degree_distribution[0], 4,
        "Degree distribution should show 4 nodes with degree 0"
    );
    ...
    for id in &["ID1", "ID2", "ID3", "ID4", "ID5", "ID6"] {
        assert!(
            !singletons.contains(&id.to_string()),
            "{} should not be identified as a singleton",
            id
        );
    }
}
