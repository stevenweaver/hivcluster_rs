...
fn test_network_construction() {
    ...
    network
        .read_from_csv_str(TEST_CSV, 0.03, InputFormat::Plain)
        .unwrap();
    ...
    assert_eq!(
        ...
        8,
        "Should have all 8 nodes in the network"
    );
...
fn test_distance_threshold() {
    let mut network = TransmissionNetwork::new();
    network
        .read_from_csv_str(TEST_CSV_THRESHOLD, 0.03, InputFormat::Plain)
        .unwrap();
    network.compute_adjacency();
...
fn test_cluster_detection() {
    ...
    assert_ne!(
        id1_cluster, id5_cluster,
        "ID1 and ID5 should be in different clusters"
    );
    assert_ne!(
        id1_cluster, id7_cluster,
        "ID1 and ID7 should be in different clusters"
    );
    assert_ne!(
        id5_cluster, id7_cluster,
        "ID5 and ID7 should be in different clusters"
    );

    // Check that clusters contain the expected nodes
    if let Some(nodes) = clusters.get(id1_cluster) {
        assert!(
            nodes.contains(&"ID1".to_string()),
            "Cluster should contain ID1"
        );
        assert!(
            nodes.contains(&"ID2".to_string()),
            "Cluster should contain ID2"
        );
        assert!(
            nodes.contains(&"ID3".to_string()),
            "Cluster should contain ID3"
        );
        assert!(
            nodes.contains(&"ID4".to_string()),
            "Cluster should contain ID4"
        );
    }

    if let Some(nodes) = clusters.get(id5_cluster) {
        assert!(
            nodes.contains(&"ID5".to_string()),
            "Cluster should contain ID5"
        );
        assert!(
            nodes.contains(&"ID6".to_string()),
            "Cluster should contain ID6"
        );
    }

    if let Some(nodes) = clusters.get(id7_cluster) {
        assert!(
            nodes.contains(&"ID7".to_string()),
            "Cluster should contain ID7"
        );
        assert!(
            nodes.contains(&"ID8".to_string()),
            "Cluster should contain ID8"
        );
    }
}
...
fn test_json_output() {
    ...
    assert_eq!(
        json.trace_results.network_summary.Nodes, 8,
        "JSON should contain 8 total nodes"
    );
    assert_eq!(
        json.trace_results.network_summary.Edges, 5,
        "JSON should contain 5 edges"
    );
    assert_eq!(
        json.trace_results.network_summary.Clusters, 3,
        "JSON should have 3 clusters"
    );
...
fn test_larger_network() {
    ...
    let mut network = TransmissionNetwork::new();
    network
        .read_from_csv_str(&csv, 0.03, InputFormat::Plain)
        .unwrap();
    network.compute_adjacency();

    // Just verify it computes without errors
    assert!(
        network.get_node_count() > 50,
        "Should have created a large network"
    );
